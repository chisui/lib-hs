...
class Singleton k => Promote (t :: Totality) (k :: k0) | k -> t where
...
instance (Known a, Known b) => Known '(a, b) where
    val _ = (val' @a, val' @b)
    val# _ = (val' @a, val' @b)
instance (Promote t0 a, Promote t1 b, MinTotality t0 t1 ~ t2) => Promote t2 (a, b) where
...
instance (Known a, Known b, Known c) => Known '(a, b, c) where
    ...
    val# _ = (val' @a, val' @b, val' @c)
instance (Promote t0 a, Promote t1 b, Promote t2 c, (t0 `MinTotality` t1 `MinTotality` t2) ~ t3) => Promote t3 (a, b, c) where
...
instance (Known a, Known b, Known c, Known d) => Known '(a, b, c, d) where
    ...
    val# _ = (val' @a, val' @b, val' @c, val' @d)
instance (Promote t0 a, Promote t1 b, Promote t2 c, Promote t3 d, (t0 `MinTotality` t1 `MinTotality` t2 `MinTotality` t3) ~ t4) => Promote t4 (a, b, c, d) where
