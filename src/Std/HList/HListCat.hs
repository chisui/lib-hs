...
instance LeftFunctor f => CatLeftFunctor'  Unconstrained (HListCat HASK) HASK (BiHListT f) where
    left' HLCId l = l
    left' (HLCVal BiHNil) BiHNil = BiHNil
    left' (HLCVal (BiHCons f fs)) (BiHCons a as) = BiHCons (left' f a) (left' (HLCVal fs) as)
instance RightFunctor f => CatRightFunctor' Unconstrained (HListCat HASK) HASK (BiHListT f) where
    right' HLCId l = l
    right' (HLCVal BiHNil) BiHNil = BiHNil
    right' (HLCVal (BiHCons f fs)) (BiHCons a as) = BiHCons (right' f a) (right' (HLCVal fs) as)
instance Bifunctor f => CatBifunctor' Unconstrained (HListCat HASK) (HListCat HASK) HASK (BiHListT f) where
