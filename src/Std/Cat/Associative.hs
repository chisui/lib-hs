...
class (EndoBifunctor cat f, Category cat) => CatAssociative (cat :: k -> k -> Type) (f :: k -> k -> k) where
    assoc :: Iso cat ((a `f` b) `f` c) (a `f` (b `f` c))
type Associative = CatAssociative HASK
...
instance CatAssociative cat f => CatAssociative (Op cat) f where
    assoc = liftIso (liftOp id) (liftOp id) assoc
